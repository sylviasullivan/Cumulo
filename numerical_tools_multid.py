...
def anomalies(ts):
    ...
    result = np.concatenate((x_Jan,x_Feb,x_Mar,x_Apr,x_May,x_Jun,x_Jul,x_Aug,x_Sep,x_Oct,x_Nov,x_Dec),axis=1)
    return result
...
def normalizer(ts):
    # Remove (linear) trend
    detrendedX = signal.detrend(ts,axis=1)
    # Compute the mean
    mean = np.mean(detrendedX,axis=1)
    # Compute the standard deviation
    std = np.std(detrendedX,axis=1)
    # normalize to zero mean and unit variance
    mean = np.reshape(mean,(mean.shape[0],1))
    std = np.reshape(std,(std.shape[0],1))
    zeroMean = detrendedX - np.repeat(mean,detrendedX.shape[1],axis=1)
    
    result = zeroMean/np.repeat(std,detrendedX.shape[1],axis=1)
    return result
...
def significantCorr(x, y, tau_max, alpha, all_corr):
    ...
    for k in arange(-tau_max,tau_max+1):
